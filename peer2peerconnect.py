...
class PeerConnection:
    ...
    Stream-safe TCP peer connection:
      - handshake: 32 bytes = 18 header + 10 zero + 4 peerId [1]
      - messages: <len:4><type:1><payload> where len excludes the 4 bytes [1]
    """
    ...
    def send_handshake(self, my_peer_id: int) -> None:
        self._sendall(self._build_handshake(my_peer_id))
    ...
    def recv_and_validate_handshake(self, expected_peer_id: Optional[int] = None) -> int:
        ...
        if data[:18] != HANDSHAKE_HEADER:
            raise IOError(f"Bad handshake header: {data[:18]!r}")

        if data[18:18 + ZERO_BITS_LEN] != (b"\x00" * ZERO_BITS_LEN):
    ...
    # len excludes the length field itself [1]

    def send_message(self, msg_type: int, payload: bytes = b"") -> None:
        if payload is None:
            payload = b""
        length = 1 + len(payload)  # type + payload [1]
        self._sendall(struct.pack(">I", length) + struct.pack(">B", msg_type) + payload)
    ...
    def send_have(self, piece_index: int) -> None:
        self.send_message(HAVE, struct.pack(">I", piece_index))  # 4-byte index [1]

    def send_bitfield(self, bitfield_bytes: bytes) -> None:
        # bitfield payload is raw bytes where bits map to piece indices [1]
        self.send_message(BITFIELD, bitfield_bytes)

    def send_request(self, piece_index: int) -> None:
        self.send_message(REQUEST, struct.pack(">I", piece_index))  # 4-byte index [1]

    def send_piece(self, piece_index: int, piece_bytes: bytes) -> None:
        self.send_message(PIECE, struct.pack(">I", piece_index) + piece_bytes)  # [1]

    # ---------------- Convenience parsers ----------------

    @staticmethod
    def parse_index_payload(payload: bytes) -> int:
        # used for HAVE and REQUEST payloads [1]
        if len(payload) != 4:
            raise ValueError("Index payload must be 4 bytes")
        (idx,) = struct.unpack(">I", payload)
        return idx

    @staticmethod
    def parse_piece_payload(payload: bytes) -> tuple[int, bytes]:
        # PIECE payload = 4-byte piece index + content [1]
        if len(payload) < 4:
